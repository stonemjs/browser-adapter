...
export class IncomingBrowserEvent extends IncomingEvent {
   ...
   * @throws {BrowserAdapterError} If the URL option is not a valid instance of URL.
   */
  protected constructor ({
    ...
    if (!(url instanceof URL)) {
      throw new BrowserAdapterError('The `url` option must be an instance of `URL`.')
    }
  ...
  */
  ...
  getCookie<TReturn extends Cookie = Cookie>(name: string, fallback?: TReturn): TReturn | undefined {
    if (!this.isValidName(name)) { throw new BrowserAdapterError('Cookie name must be a non-empty string.') }
